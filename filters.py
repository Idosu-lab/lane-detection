...
def enhance_contrast(frame):
    ...
    l, a, b = cv2.split(lab)
    clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))  # More moderate contrast adjustment
    cl = clahe.apply(l)
...
def detect_edges(frame):
    ...
    edges = cv2.Canny(gray, 50, 120)  # Adjusted Canny edge detection thresholds
    return edges
...
def filter_lines_by_slope(lines):
    ...
    if lines is not None:
        for line in lines:
            x1, y1, x2, y2 = line[0]
            slope = (y2 - y1) / (x2 - x1) if x2 != x1 else 999.0  # Avoid division by zero
            if 0.5 < abs(slope) < 2:  # Filter slopes within a valid range
                filtered_lines.append(line)
    return filtered_lines

def detect_lines_and_calculate_space(edges, original_frame):
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=30, minLineLength=40, maxLineGap=100)
    lines = filter_lines_by_slope(lines)  # Filter out noise based on slope
    line_image = np.zeros_like(original_frame)
    lane_lines = []
    if lines is not None:
        for line in lines:
            x1, y1, x2, y2 = line[0]
    ...
    combined_image = cv2.addWeighted(original_frame, 0.8, line_image, 1, 0)

    # Calculate lane space (if at least two lines detected)
    if len(lane_lines) >= 2:
        left_line = lane_lines[0]
        right_line = lane_lines[1]
        midpoint_left = (left_line[0][0] + left_line[1][0]) // 2
        midpoint_right = (right_line[0][0] + right_line[1][0]) // 2
        lane_width = abs(midpoint_right - midpoint_left)
...
def process_frame(frame):
    ...
    return detect_lines_and_calculate_space(edges, frame)
